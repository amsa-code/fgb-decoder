...
public class Common
{
	...
	{
	case 0:
	    maritimeEmergencyCodes = new HexAttribute(name, 109, 112, mec+"(Unspecified distress) "+binCode.substring(109, 113), "");
	    break;
	case 1:
	    maritimeEmergencyCodes = new HexAttribute(name, 109, 112, mec+"(Fire/explosion) "+binCode.substring(109, 113), "");
	    break;
	case 2:
	    maritimeEmergencyCodes = new HexAttribute(name, 109, 112, mec+"(Flooding) "+binCode.substring(109, 113), "");
	    break;
	case 3:
	    maritimeEmergencyCodes = new HexAttribute(name, 109, 112, mec+"(Collision) "+binCode.substring(109, 113), "");
	    break;
	case 4:
	    maritimeEmergencyCodes = new HexAttribute(name, 109, 112, mec+"(Grounding) "+binCode.substring(109, 113), "");
	    break;
	case 5:
	    maritimeEmergencyCodes = new HexAttribute(name, 109, 112, mec+"(Listing, in danger of capsizing) "+binCode.substring(109, 113), "");
	    break;
	case 6:
	    maritimeEmergencyCodes = new HexAttribute(name, 109, 112, mec+"(Sinking) "+binCode.substring(109, 113), "");
	    break;
	case 7:
	    maritimeEmergencyCodes = new HexAttribute(name, 109, 112, mec+"(Disabled and adrift) "+binCode.substring(109, 113), "");
	    break;
	case 8:
	    maritimeEmergencyCodes = new HexAttribute(name, 109, 112, mec+"(Abandoning ship) "+binCode.substring(109, 113), "");
	    break;
	default:
    ...
    public static HexAttribute fireFlag(String binCode, int s) {
	String v = "";
	String e = "";
	...
	// This is grouped under "Nature of Distress" with the identation of 30 white spaces
	//return new HexAttribute("                              ", s, v, e);
	return new HexAttribute("Fire Flag", s, v, e);
    }

    public static HexAttribute medicalHelpFlag(String binCode, int s) {
	String v = "";
	String e = "";
	if (binCode.charAt(s) == '1') {
	    v = "YES";
    ...
    public static HexAttribute disabledFlag(String binCode, int s) {
	String v = "";
	String e = "";
	if (binCode.charAt(s) == '1') {
	    v = "YES";
    ...
    public static HexAttribute emergencySpare(String binCode, int s) {
	String v = "";
	String e = "";
    ...
    public static final class Position {
        ...
        public Position(int latSeconds, int lonSeconds, String latText, String lonText) {
    ...
    //TODO use this wherever lat/long decoding happens because there is a lot of copy and paste around! Dave Moten April 11 2019
    /**
     * @param binCode input binary string
     * @param start start index
     * @param length length of position encoding 
     * @param secondsPerUnit factor to use when converting a decimal number to seconds
     * @return position
     */
    public static Position position(String binCode, int start, int length, int secondsPerUnit) {
        final int lonLength;
        ...
        final String lonText;
        {
            ...
            int code = Conversions
                    .binaryToDecimal(lonBits);
            final int codeSeconds = code * secondsPerUnit;
            final int deg = codeSeconds / 3600;
            final int min = codeSeconds % 3600 / 60;
            int seconds = deg * 3600 + min * 60;
