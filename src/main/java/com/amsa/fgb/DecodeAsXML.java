...
public class DecodeAsXML implements DecodeFilter {
	 ...
	 * = desc.length(); if (len > 0) { String tag = ""; for (int j=0; j<len;
	 * j++) { char c = desc.charAt(j); if (c != ' ' && c!= '.') { tag = tag + c;
	 * } } line = "<" + tag + ">"; if (pos.equals("b0")) { pos = ""; } line =
	 * line + "<Position>" + pos + "</Position>"; line = line + "<Value>" + val
	 * + "</Value>"; if (err.length() > 0) { line = line + "<Error>" + err +
	 * "</Error>"; } line = line + "</" + tag + ">\n"; } data = data + line; }
	 * 
	 * data = data + "</Hexdecode>\n";
	 * 
