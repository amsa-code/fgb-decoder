...
class LocationSpare extends BeaconProtocol {
    ...
    public Vector<HexAttribute> decode(String hexStr) {
		...
		result.add(new HexAttribute("ID+POSN", 41, 85, binCode
				.substring(41, 86), ""));
		...
		if (hexStr.length() > 15) {
			...
			if (this.isLongMessage(binCode)) {
				result.add(new HexAttribute("PDF-2", 107, 132, binCode
						.substring(113, 133), ""));
				result.add(new HexAttribute("BCH-2", 133, 144, binCode
						.substring(133, 145), ""));
			} else
				result.add(new HexAttribute("Supplementary Data", 107, 112,
						binCode.substring(107, 113), ""));
